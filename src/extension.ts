...
const SECRET_KEY = 'codexClone.openaiApiKey';
...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand('codexClone.setApiKey', async () => {
      ...
      vscode.window.showInformationMessage('Codex Clone: API key saved securely in VS Code.');
    }),
...
async function openChat(context: vscode.ExtensionContext) {
  ...
  panel.webview.onDidReceiveMessage(async (message) => {
    if (message?.type === 'setApiKey') {
      ...
      panel.webview.postMessage({ type: 'status', text: 'API key updated.' });
      return;
    }
    ...
    if (!apiKey) {
      panel.webview.postMessage({ type: 'agentError', text: 'OpenAI API key is not set. Use Command Palette → Codex Clone: Set OpenAI API Key.' });
      return;
    }

    busy = true;
    panel.webview.postMessage({ type: 'agentState', state: 'working' });
    panel.webview.postMessage({ type: 'agentActivity', items: ['Request received', 'Preparing Codex context'] });
    ...
    try {
      ...
      const reply = await callCodex(apiKey, text, history, workspaceName, context);
      history.push({ role: 'user', content: text }, { role: 'assistant', content: reply });

      panel.webview.postMessage({ type: 'agentActivity', items: ['Request sent to Codex', 'Response received'] });
...
async function callCodex(
  ...
  const reasoningEffort = config.get<'low' | 'medium' | 'high' | 'xhigh'>('reasoningEffort', 'medium');

  const prior = history.slice(-12).map((message) => ({
    role: message.role,
  ...
  const input = [
    {
      role: 'developer',
      content: `You are Codex Clone, a helpful coding agent running inside VS Code. The current workspace is "${workspaceName}". For now you are in chat-only mode: explain code clearly and propose changes, but do not claim to have edited or executed files.`,
    },
    ...prior,
